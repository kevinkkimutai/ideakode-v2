...
import { useDeleteTicketMutation, useGetAllTicketsMutation } from "@/redux/actions/supportTicketsActions";
import { setTickets } from "@/redux/reducers/supportTicketsReducers";
...
export default function SupportTickets() {
  ...
  const [getSupportTickets] = useGetAllTicketsMutation();
  ...
  useEffect(() => {
    if (searchTerm) {
      const filtered = supportTickets.filter((category) =>
        category.fullname.toLowerCase().includes(searchTerm.toLowerCase())
      );
      setFilteredTickets(filtered);
    } else {
      setFilteredTickets(supportTickets);
    }
  }, [searchTerm, supportTickets]);
...
const handleDelete = async (categoryId) => {
  try {
    const response = await deleteTicket({ id: categoryId });
    if (response.data) {
      toast.success(response.data.message);
      setTickets((prev) => prev.filter((cat) => cat.id !== categoryId));
    }
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            ...
            {loading
              ...
              : filteredTickets?.map((category, index) => (
                  <tr
                    key={index}
                    className="bg-white border-b dark:bg-white/[0.03] dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800"
                  >
                    ...
                    <td className="px-6 py-4 text-gray-800 whitespace-nowrap dark:text-white">
                    ...
                    <td className="px-6 py-4">
                      <div
                        className={`flex text-xs font-[500] items-center `}
                      >
                      {category.updatedAt ? formatDistanceToNow(new Date(category.updatedAt), { addSuffix: true }) : "N/A"}
                      </div>
                    </td>
                    <td className="px-6 py-4">
